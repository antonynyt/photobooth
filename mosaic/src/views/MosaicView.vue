...
<template>
  ...
  <div class="pixel" v-for="n in (imageHeight/pixelHeight)*(imageWidth/pixelWidth)">
    <img class="user-img" :id="`user-img-${n}`" :src="`data:image/png;base64,${photos[n-1]?.img}`" :style="`top: ${((n-1)*pixelHeight)%imageHeight}vw; left:  ${Math.floor((n-1)/(imageHeight/pixelHeight))*pixelWidth}vw; opacity: 0.3; width: ${pixelWidth}vw; height: ${pixelHeight}vw`"></img>
